...
export async function initializeAppLogger (): Promise<void> {
  try {
        ...
        betterStack: frontendConfig.logging.betterStack
          ? {
              ...
              tokenPreview: frontendConfig.logging.betterStack.token
                ? `${frontendConfig.logging.betterStack.token.slice(0, 8)}...`
                : 'Not set',
            }
          : 'Not configured',
      },
    ...
    if (import.meta.env.MODE === 'development') {
      ...
      console.log('VITE_BETTERSTACK_TOKEN:', import.meta.env.VITE_BETTERSTACK_TOKEN ? `${import.meta.env.VITE_BETTERSTACK_TOKEN.slice(0, 8)}...` : 'Not set')
        ...
        betterStack: response.config.logging.betterStack
          ? {
              ...
              tokenPreview: (response.config.logging.betterStack as any)?.sourceToken
                ? `${(response.config.logging.betterStack as any).sourceToken.slice(0, 8)}...`
                : 'Not set',
            }
          : 'Not configured',
      },
    ...
    const hasToken = finalBetterStackConfig
      ? !!(finalBetterStackConfig as any)?.sourceToken || !!(finalBetterStackConfig as any)?.token
      : false
