...
<template>
  <v-container fluid>
      ...
      <v-card v-if="task" class="mb-6">
        <v-card-title class="bg-primary">
          <div class="d-flex justify-space-between align-center w-100">
            ...
            <v-btn color="white" icon="mdi-graph" size="small" variant="text" @click="openTaskGraphDialog">
        ...
        <v-card-text>
          <v-row class="pt-4">
            <!-- Basic Info -->
            <v-col cols="12" md="6">
              <v-text-field label="Task Name" :model-value="task.name" prepend-icon="mdi-clipboard-text" readonly
                variant="outlined" />
            </v-col>
            <v-col cols="12" md="6">
              <v-text-field label="Project" :model-value="task.project?.name || 'Unknown'"
                prepend-icon="mdi-clipboard-outline" readonly variant="outlined" />
            </v-col>

            <!-- Description -->
            <v-col cols="12">
              <v-textarea label="Description" :model-value="task.description || 'No description provided'"
                prepend-icon="mdi-text-box" readonly rows="2" variant="outlined" />
            </v-col>

            <!-- Status Info -->
            <v-col cols="12" md="4">
              <v-text-field label="Status" :model-value="task.status?.name || 'Not set'" prepend-icon="mdi-flag"
                readonly variant="outlined" />
            </v-col>
            <v-col cols="12" md="4">
              <v-text-field label="Priority" :model-value="task.priority?.name || 'Not set'"
                prepend-icon="mdi-priority-high" readonly variant="outlined" />
            </v-col>
            <v-col cols="12" md="4">
              <v-text-field label="Complexity" :model-value="task.complexity?.name || 'Not set'"
                prepend-icon="mdi-chart-line" readonly variant="outlined" />
            </v-col>
          </v-row>
        </v-card-text>
      </v-card>
        ...
        <v-card class="mb-6">
          ...
          <v-card-text>
            ...
            <v-list v-else class="pa-0">
              <v-list-item v-for="report in sortedProgressReports" :key="report.id" class="px-0 py-3 border-b"
                elevation="0">
                <template #prepend>
      ...
      <v-col cols="12" lg="6">
        <v-card class="mb-6">
          ...
          <v-card-text>
            ...
            <v-list v-else class="pa-0">
              <v-list-item v-for="report in sortedStatusReports" :key="report.id" class="px-0 py-3 border-b"
                elevation="0">
                <template #prepend>
                ...
                <div class="flex-grow-1">
                  ...
                  <v-list-item-subtitle class="mb-2">
                    ...
                    <div class="d-flex align-center gap-2 flex-wrap">
                      <v-chip v-if="report.creator" size="small" variant="outlined" color="primary"
                        prepend-icon="mdi-account">
                        {{ report.creator.firstName }} {{ report.creator.lastName }}
                      </v-chip>

                      <v-chip size="small" variant="outlined" color="info" prepend-icon="mdi-calendar-clock">
    ...
    <v-dialog v-model="showAssigneeCreateDialog" max-width="500" persistent>
      <TaskAssigneeCreateDialog :available-staff="availableStaff" :current-assignees="assignees"
        @cancel="showAssigneeCreateDialog = false" @created="handleAssigneeCreated" />
    </v-dialog>

    <v-dialog v-model="showPredecessorCreateDialog" max-width="600" persistent>
      <TaskPredecessorCreateDialog :available-tasks="availableTasks" :current-predecessors="predecessors"
        :current-task-id="taskId" @cancel="showPredecessorCreateDialog = false" @created="handlePredecessorCreated" />
    </v-dialog>

    <v-dialog v-model="showChildTaskCreateDialog" max-width="700" persistent>
      <TaskChildCreateDialog :available-complexities="availableComplexities" :available-priorities="availablePriorities"
        :available-statuses="availableStatuses" :parent-task-id="taskId" :project-id="task?.projectId || ''"
        @cancel="showChildTaskCreateDialog = false" @created="handleChildTaskCreated" />
    </v-dialog>

    <v-dialog v-model="showProgressReportCreateDialog" max-width="500" persistent>
      <TaskProgressCreateDialog :eligible-staff="eligibleStaff" :task-id="taskId"
        @cancel="showProgressReportCreateDialog = false" @created="handleProgressReportCreated" />
    </v-dialog>

    <v-dialog v-model="showProgressReportEditDialog" max-width="500" persistent>
      <TaskProgressEditDialog v-if="selectedProgressReport" :progress-report="selectedProgressReport"
        :eligible-staff="eligibleStaff" @cancel="showProgressReportEditDialog = false"
        @updated="handleProgressReportUpdated" />
    </v-dialog>

    <v-dialog v-model="showStatusReportCreateDialog" max-width="600" persistent>
      <TaskStatusReportCreateDialog :eligible-staff="eligibleStaff" :task-id="taskId"
        @cancel="showStatusReportCreateDialog = false" @created="handleStatusReportCreated" />
    </v-dialog>

    <v-dialog v-model="showStatusReportEditDialog" max-width="600" persistent>
      <TaskStatusReportEditDialog v-if="selectedStatusReport" :status-report="selectedStatusReport"
        :eligible-staff="eligibleStaff" @cancel="showStatusReportEditDialog = false"
